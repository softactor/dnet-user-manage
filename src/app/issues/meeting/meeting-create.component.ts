  ...
  public onFormSubmit(fields, type) {
    if (this.form_type) {
      ...
      if (this.defaultDate) {
        ...
        const postString = 'issue_discussed=' + ((fields.issue_discussed === undefined) ? '' : fields.issue_discussed)
          + '&name=' + ((fields.name === undefined) ? '' : fields.name)
          + '&organization=' + ((fields.organization === undefined) ? '' : fields.organization)
        ...
        this._service.create(postString, this.authorizationKey,
          'issues/meeting/create').subscribe(response => {
            ...
            const postMenuString = 'name=' + this.form_type
              ...
              + '&parent_id=' + 5
              + '&url=meeting-list/' + this.form_type
              + '&type=' + this.form_type
            this._service.create(postMenuString, this.authorizationKey, 'menumanagment/leftmenu/create').subscribe(menu_response => {
                ...
                this.router.navigate(['meeting-list/' + this.form_type]);
                // location.reload();
              },
              error => {
