...
import { OtherActivityModel } from './other-activity.model';
...
export class OtherActivityCreateComponent implements OnInit {
  otherActivity: OtherActivityModel[] = [];
  similarTypes;
  inputFields;
  formData;
  authorizationKey;
  ...
  ngOnInit() {
    ...
    const residenceObj = new OtherActivityModel();
    // @ts-ignore
    this.otherActivity.push(residenceObj);
  ...
  public onFormSubmit(fields, type) {
    ...
    const postString  =  'activity=' + ((fields.activity === undefined) ? '' : fields.activity)
  ...
  public copyForm(e) {
    if (this.form_type) {
      if (this.similarTypes.indexOf(this.form_type) === -1) {
        this.otherActivity  = [];
        this.similarTypes.push(this.form_type);
        const companyObj = new OtherActivityModel();
        // @ts-ignore
        this.otherActivity.push(companyObj);
      }else {
